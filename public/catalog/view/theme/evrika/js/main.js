// Данные каталога: сначала берём из OcStore (через Twig), иначе — встроенные
const catalogData = (window.EVRIKA_CATALOG && window.EVRIKA_CATALOG.length) ? window.EVRIKA_CATALOG : [
  {
...
/* ─── Карточки товаров: qty ± и «В корзину» (сетка + список) ─── */
(function initProdCards() {
  document.querySelectorAll('.prod-card, .prod-list-item').forEach(function(card) {
    ...
    if (minusBtn && plusBtn && input) {
      ...
      function updateMinus() { minusBtn.disabled = +input.value <= minVal; }

      minusBtn.addEventListener('click', function() {
        var v = +input.value;
        if (v > minVal) { input.value = v - 1; updateMinus(); }
      });
